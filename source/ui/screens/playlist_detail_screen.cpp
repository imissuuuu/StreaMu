...
std::string PlaylistDetailScreen::update(AppContext &ctx, u32 kDown, u32 kHeld,
                                         u32 kRepeat) {
  LightLock_Lock(&ctx.lock);
  ...
  if ((kHeld & KEY_TOUCH) && ctx.touch_state.is_touching) {
    ...
    if (ctx.touch_state.is_dragging) {
      ...
      float max_scroll =
          std::max(0.0f, (float)(track_count - BTM_MAX_VISIBLE_2ROW) *
                             BTM_ITEM_HEIGHT_2ROW);
      if (ctx.scroll_offset_y < min_scroll)
        ctx.scroll_offset_y = min_scroll;
  ...
  if (ctx.selected_index == 0) {
    LightLock_Lock(&ctx.lock);
    bool is_mode_btn =
        !ctx.g_tracks.empty() && ctx.g_tracks[0].id == "MODE_BTN";
    LightLock_Unlock(&ctx.lock);
    if (is_mode_btn) {
  ...
  if (kDown & KEY_A) {
    ...
    if (has_tracks) {
      if (sel_id == "MODE_BTN") {
        return mode_btn_focus_ == 0 ? "start_shuffle_playback"
                                    : "start_order_playback";
      } else if (ctx.playing_id == sel_id) {
        return "toggle_pause";
      } else {
        return "start_playback_from_playlist";
