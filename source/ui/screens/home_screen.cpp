...
static inline u32 darken_color(u32 c, float factor) {
  return C2D_Color32((u8)(col_r(c) * factor), (u8)(col_g(c) * factor),
                     (u8)(col_b(c) * factor), 255);
}

static void draw_gradient(float x, float y, float w, float h, u32 top_col,
                          u32 bot_col) {
...
static void draw_press_overlay(float x, float y, float w, float h,
                               u32 inset_col) {
...
HomeScreen::Rect HomeScreen::get_tile_rect(TileId id) const {
  switch (id) {
  case TILE_QA:
    return {HOME_LEFT_X, HOME_AREA_TOP, HOME_COL_W, HOME_QA_H};
  case TILE_PLAYING:
    return {HOME_LEFT_X, HOME_QA_H, HOME_COL_W, HOME_AREA_BOTTOM - HOME_QA_H};
  case TILE_SEARCH:
    return {HOME_RIGHT_X, HOME_AREA_TOP, HOME_COL_W, HOME_TILE_H};
  case TILE_PLAYLISTS:
    return {HOME_RIGHT_X, HOME_TILE_H, HOME_COL_W, HOME_TILE_H};
  case TILE_SETTINGS:
    return {HOME_RIGHT_X, HOME_TILE_H * 2.0f, HOME_COL_W,
            HOME_AREA_BOTTOM - HOME_TILE_H * 2.0f};
  default:
    return {0, 0, 0, 0};
  }
}
...
void HomeScreen::draw_bottom(const RenderContext &ctx, UIManager &ui_mgr) {
  ...
  u32 inset_col =
      darken_color(accent, 0.50f); // Darkened accent for inset border
  ...
  auto draw_tile = [&](TileId id, const char *label) {
    ...
    C2D_DrawText(&text, C2D_WithColor, tx + HOME_CURSOR_W, ty, 0, font, font,
                 col);
  };
  ...
  if (is_playing_visible(ctx)) {
    const char *playing_label =
        !ctx.active_playlist_id.empty() ? "Playing" : "Search Result";
  ...
  // Left column: Quick Access tile
  {
    ...
    if (qa_tile_sel && !is_pressed) {
      C2D_TextParse(&text, buf, ">");
      C2D_DrawText(&text, C2D_WithColor, hdr_x, hdr_y, 0, HOME_QA_HEADER_FONT,
                   HOME_QA_HEADER_FONT, hdr_col);
    }
    C2D_TextParse(&text, buf, "Quick Access");
    C2D_DrawText(&text, C2D_WithColor, hdr_x + HOME_CURSOR_W, hdr_y, 0,
                 HOME_QA_HEADER_FONT, HOME_QA_HEADER_FONT, hdr_col);
    ...
    for (int i = 0; i < (int)qa_slots_.size(); ++i) {
      ...
      float text_y =
          slot_y + (HOME_QA_SLOT_H - HOME_QA_SLOT_FONT * 24.0f) / 2.0f;
      float slot_x = r.x + HOME_TILE_PAD + HOME_CURSOR_W + press_off;

      // ▶ cursor for selected QA slot
      if (slot_sel) {
        C2D_TextParse(&text, buf, ">");
        C2D_DrawText(&text, C2D_WithColor, r.x + HOME_TILE_PAD + press_off,
                     text_y + press_off, 0, HOME_QA_SLOT_FONT,
                     HOME_QA_SLOT_FONT, slot_col);
      }

      C2D_TextParse(&text, buf, qa_slots_[i].label.c_str());
      C2D_DrawText(&text, C2D_WithColor, slot_x, text_y + press_off, 0,
                   HOME_QA_SLOT_FONT, HOME_QA_SLOT_FONT, slot_col);
...
std::string HomeScreen::update(AppContext &ctx, u32 kDown, u32 kHeld,
                               u32 kRepeat) {
  ...
  if (kDown & KEY_TOUCH) {
    ...
    if (py < (int)PlayBar::BAR_Y) {
      for (int id = 0; id < TILE_COUNT; ++id) {
        ...
        if (tid == TILE_PLAYING && !is_playing_visible(ctx))
          continue;
        Rect r = get_tile_rect(tid);
        if (px >= (int)r.x && px < (int)(r.x + r.w) && py >= (int)r.y &&
            py < (int)(r.y + r.h)) {
  ...
  if (!(kHeld & KEY_TOUCH) && ctx.touch_state.is_touching) {
    ...
    if (from_seekbar) {
      ...
      if (total_secs > 0 && !ctx.playing_id.empty()) {
        ...
        if (ratio > 0.97f)
          ratio = 1.0f;
        ctx.seek_target_seconds =
            (ratio >= 1.0f) ? total_secs - 1 : (int)(ratio * total_secs);
        return "seek";
      }
    } else if (was_tap) {
      // PlayBar touch
      if (ty >= (int)PlayBar::BAR_Y) {
        ...
        for (int id = 0; id < TILE_COUNT; ++id) {
          ...
          Rect r = get_tile_rect(tid);
          if (tx >= (int)r.x && tx < (int)(r.x + r.w) && ty >= (int)r.y &&
              ty < (int)(r.y + r.h)) {
            ...
            if (tid == TILE_QA) {
              ...
              float slot_start =
                  qr.y + HOME_TILE_PAD + HOME_QA_HEADER_FONT * 24.0f + 4.0f;
              ...
              if (tid == TILE_PLAYLISTS) {
                sel_col_ = 1;
                sel_row_ = 1;
              }
              if (tid == TILE_SETTINGS) {
                sel_col_ = 1;
                sel_row_ = 2;
              }
              if (tid == TILE_PLAYING) {
...
std::string HomeScreen::get_tile_action(TileId id, AppContext &ctx) {
  ...
  case TILE_PLAYLISTS:
    return "trigger_playlists";
  case TILE_SETTINGS:
    return "trigger_settings";
  case TILE_PLAYING:
    if (is_playing_visible(ctx))
      return "trigger_playing";
    return "";
  case TILE_QA:
    if (qa_focused_ && qa_sel_ >= 0 && qa_sel_ < (int)qa_slots_.size()) {
      if (qa_slots_[qa_sel_].playlist_id.empty()) {
