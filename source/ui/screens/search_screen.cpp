...
std::string SearchScreen::update(AppContext &ctx, u32 kDown, u32 kHeld,
                                 u32 kRepeat) {
  touchPosition touch;
  hidTouchRead(&touch);
  ...
  if ((kHeld & KEY_TOUCH) && ctx.touch_state.is_touching) {
    ...
    if (ctx.touch_state.is_dragging &&
      ...
      int visible =
          (int)((PlayBar::BAR_Y - BTM_LIST_START_Y) / BTM_ITEM_HEIGHT_2ROW);
      float max_scroll =
          ((int)ctx.search_tracks.size() - visible) * BTM_ITEM_HEIGHT_2ROW;
  ...
  if (!ctx.search_tracks.empty()) {
    ...
    if (moved) {
      ...
      if (ctx.selected_index < scroll_items)
        ...
        ctx.scroll_offset_y =
            (ctx.selected_index - visible + 1) * BTM_ITEM_HEIGHT_2ROW;
      float max_scroll =
          ((int)ctx.search_tracks.size() - visible) * BTM_ITEM_HEIGHT_2ROW;
      if (ctx.scroll_offset_y < 0.0f)
        ctx.scroll_offset_y = 0.0f;
...
void SearchScreen::draw_bottom(const RenderContext &ctx, UIManager &ui_mgr) {
  ...
  // Replace LOOP zone with hamburger nav button
  {
    ...
    C2D_DrawRectSolid(0, PlayBar::BAR_Y + PlayBar::SEEK_H, 0,
                      (int)PlayBar::ZONE_1, PlayBar::BAR_H - PlayBar::SEEK_H,
                      bar_bg);
    // Draw ☰ centered in the zone
    float cy = PlayBar::BAR_Y + PlayBar::SEEK_H +
               (PlayBar::BAR_H - PlayBar::SEEK_H) * 0.5f;
    float ix =
