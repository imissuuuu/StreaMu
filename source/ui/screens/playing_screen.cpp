...
void PlayingScreen::draw_bottom(const RenderContext &ctx, UIManager &ui_mgr) {
  // --- Track list ---
  draw_playing_style_list(ctx.playing_tracks, ctx, ui_mgr);
...
// Input handling
// ============================================================
std::string PlayingScreen::update(AppContext &ctx, u32 kDown, u32 kHeld,
                                  u32 kRepeat) {
  touchPosition touch;
  hidTouchRead(&touch);
  ...
  if ((kHeld & KEY_TOUCH) && ctx.touch_state.is_touching) {
    ...
    if (ctx.touch_state.is_dragging &&
      ...
      if (max_scroll <= 0.0f)
        ctx.scroll_offset_y = 0.0f;
      else if (ctx.scroll_offset_y > max_scroll)
        ctx.scroll_offset_y = max_scroll;
    }
  }
  if (!(kHeld & KEY_TOUCH) && ctx.touch_state.is_touching) {
    int tx = ctx.touch_state.current_x;
    int ty = ctx.touch_state.current_y;
    // True if touch started in the seek bar zone (handles both tap and drag)
    bool from_seekbar =
        ctx.touch_state.start_y >= (int)PlayBar::BAR_Y &&
    ...
    if (from_seekbar) {
      // Tap or horizontal drag on seek bar → seek to released x position
      ...
      if (total_secs > 0 && !ctx.playing_id.empty()) {
        ...
        if (ratio < 0.03f)
          ratio = 0.0f; // snap to start (handles touch panel dead zone)
        if (ratio > 0.97f)
          ratio = 1.0f; // snap to end
      ...
      if (tx >= (int)HOME_BTN_X && ty >= (int)HOME_BTN_Y &&
          ty < (int)PlayBar::BAR_Y) {
        return "trigger_home";
  ...
  // D-pad cursor movement + scroll follow (same behavior as search/playlist
  // screens)
