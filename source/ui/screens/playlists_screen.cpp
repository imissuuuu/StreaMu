...
std::string PlaylistsScreen::update(AppContext &ctx, u32 kDown, u32 kHeld,
                                    u32 kRepeat) {
  ...
  if ((kHeld & KEY_TOUCH) && ctx.touch_state.is_touching) {
    ...
    if (ctx.touch_state.is_dragging) {
      ...
      float max_scroll =
          (playlist_count - BTM_MAX_VISIBLE_2ROW) * BTM_ITEM_HEIGHT_2ROW;
      if (max_scroll < 0)
        max_scroll = 0;
...
void PlaylistsScreen::draw_bottom(const RenderContext &ctx, UIManager &ui_mgr) {
  ...
  for (int i = start_idx;
    ...
    C2D_DrawText(&text, C2D_WithColor, BTM_MARGIN_X, y_pos, 0, FONT_SM, FONT_SM,
                 color);
