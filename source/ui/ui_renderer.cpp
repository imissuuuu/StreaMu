...
void UIRenderer::draw_top_screen(const RenderContext &ctx) {
  ...
  // --- WiFi Icon: 3 ascending bars, bottom-aligned ---
  {
    ...
    if (wifi == 0) {
      ...
      C2D_DrawText(&text, C2D_WithColor, wx + 2, TOP_SYS_Y, 0, FONT_SM, FONT_SM,
                   col);
    } else {
      for (int i = 0; i < 3; i++) {
  ...
  // --- Battery Icon: outline + terminal + gauge segments ---
  {
    ...
    if (charging) {
      ...
      C2D_DrawText(&text, C2D_WithColor, bx + frame_w + 4, TOP_SYS_Y, 0,
                   FONT_SM, FONT_SM, col);
    }
  }
  ...
  // --- Clock ---
  {
    ...
    char time_str[8];
    if (tm)
      snprintf(time_str, sizeof(time_str), "%02d:%02d", tm->tm_hour,
               tm->tm_min);
    else
      snprintf(time_str, sizeof(time_str), "--:--");
    C2D_TextParse(&text, m_ui.get_text_buf(), time_str);
    C2D_DrawText(&text, C2D_WithColor, clock_x, TOP_SYS_Y, 0, FONT_SM, FONT_SM,
                 col);
  }
  ...
  C2D_DrawText(&text, C2D_WithColor, 400.0f - TOP_MARGIN_X - ver_w, TOP_SYS_Y,
               0, FONT_SM, FONT_SM, m_colors.accent_text);

  // --- Server Status ---
  if (!ctx.is_server_connected) {
    C2D_TextParse(&text, m_ui.get_text_buf(), "[ SERVER OFFLINE ]");
    C2D_DrawText(&text, C2D_WithColor, TOP_OFFLINE_X, TOP_SYS_Y, 0, FONT_SM,
                 FONT_SM, m_colors.warn);
  }
  ...
  if (title_state == STATE_HOME)
    top_title = "HOME";
  else if (title_state == STATE_SEARCH)
    top_title = "SEARCH";
  else if (title_state == STATE_PLAYLISTS)
    top_title = "PLAYLISTS";
  else if (title_state == STATE_PLAYLIST_DETAIL)
    top_title = "PLAYLIST DETAIL";
  else if (title_state == STATE_PLAYING_UI)
    top_title = "";
  else if (title_state == STATE_SETTINGS)
    top_title = "SETTINGS";
  ...
  if (!ctx.g_status_msg.empty()) {
    ...
    C2D_DrawRectSolid(badge_x, badge_y, 0, badge_w, badge_h,
                      C2D_Color32(30, 30, 30, 160));
    C2D_DrawText(&text, C2D_WithColor, badge_x + pad_x, badge_y + pad_y, 0,
                 FONT_SM, FONT_SM, C2D_Color32(220, 220, 220, 255));
  }

  // --- Thumbnail (all screens except Settings) ---
  if (ctx.current_state != STATE_SETTINGS && m_thumbnail &&
      m_thumbnail->is_loaded()) {
  ...
  if (!ctx.playing_title_lines.empty()) {
    ...
    u32 win_bg = (ctx.config.mode == THEME_DARK)
                     ? C2D_Color32(55, 55, 55, 0xD0)
                     : C2D_Color32(230, 230, 230, 0xD0);
    C2D_DrawRectSolid(0, TOP_NOW_PLAYING_WIN_Y, 0, TOP_NOW_PLAYING_WIN_W,
                      TOP_NOW_PLAYING_WIN_H, win_bg);
    ...
    for (const auto &line : ctx.playing_title_lines) {
      ...
      C2D_DrawText(&text, C2D_WithColor, TOP_MARGIN_X, draw_y, 0, FONT_SM,
                   FONT_SM, m_colors.text_body);
    ...
    if (!ctx.active_playlist_name.empty() || !ctx.playing_meta.empty()) {
      ...
      C2D_DrawText(&text, C2D_WithColor, TOP_MARGIN_X + 12.0f, draw_y, 0,
                   FONT_SM, FONT_SM, m_colors.text_body);
      draw_y += TOP_PLAYING_LINE;
    }
  }
}
...
void UIRenderer::draw_popup_overlay(const RenderContext &ctx) {
  ...
  if (ctx.current_state == STATE_POPUP_PLAYLIST_ADD) {
    ...
    // otherwise
    {
      ...
      if (!ctx.config.quick_access_ids.empty()) {
        ...
        while (start < ctx.config.quick_access_ids.size()) {
          ...
          if (!id.empty()) {
            qa_count++;
            if (id == ctx.selected_playlist_id)
              in_qa = true;
          }
    ...
    if (ctx.previous_state == STATE_PLAYLIST_DETAIL ||
    ...
    if (found_track) {
      ...
      std::string ch = t.uploader.length() > 30
                           ? t.uploader.substr(0, 30) + "..."
                           : t.uploader;
  ...
  // Popup Background
  C2D_DrawRectSolid(POPUP_MARGIN_X, box_y, 0, POPUP_WIDTH, box_h,
                    m_colors.popup_bg);
  // Popup Header
  C2D_DrawRectSolid(POPUP_MARGIN_X, box_y, 0, POPUP_WIDTH, POPUP_HEADER_H,
                    m_colors.popup_header);

  C2D_TextParse(&text, m_ui.get_text_buf(), popup_title.c_str());
  C2D_DrawText(&text, C2D_WithColor, POPUP_MARGIN_X + 4, box_y + 6, 0, FONT_SM,
               FONT_SM, m_colors.popup_title);
  ...
  if (start_idx + max_items > (int)options.size())
    start_idx = (int)options.size() - max_items;
  if (start_idx < 0)
    start_idx = 0;

  for (int i = start_idx; i < (int)options.size() && i < start_idx + max_items;
       i++) {
    float y_pos = box_y + POPUP_HEADER_H + 4 + (i - start_idx) * POPUP_ITEM_H;
    if (i == ctx.popup_selected_index) {
      C2D_DrawRectSolid(POPUP_MARGIN_X + 2, y_pos - 2, 0, POPUP_WIDTH - 4,
                        POPUP_ITEM_H, m_colors.accent);
    }
    ...
    u32 color = (i == ctx.popup_selected_index)
                    ? m_colors.accent_text
                    : (is_grayed ? m_colors.text_dim : m_colors.popup_text);
    C2D_TextParse(&text, m_ui.get_text_buf(), options[i].c_str());
    C2D_DrawText(&text, C2D_WithColor, POPUP_INNER_X, y_pos, 0, FONT_SM,
                 FONT_SM, color);
  }
}
