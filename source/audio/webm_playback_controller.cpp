    ...
    case WebmPlaybackStage::Prebuffering:
      ...
      decision.max_decode_buffers = kAggressiveMaxDecodeBuffers;
      {
        ...
        if (input.user_paused) {
          decision.hold_reason = WebmPrebufferHoldReason::UserPaused;
        } else if (!queue_ready) {
          decision.hold_reason = WebmPrebufferHoldReason::QueueBelowTarget;
        } else if (!bytes_ready) {
          decision.hold_reason = WebmPrebufferHoldReason::BytesBelowRelease;
        } else {
          decision.hold_reason = WebmPrebufferHoldReason::None;
        }
        decision.release_prebuffer =
            !input.user_paused && queue_ready && bytes_ready;
      }
      return decision;
